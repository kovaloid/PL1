...
#define  NOP 14                                   /*кол-во обрабатываемых   */
						  /* команд                 */
...
int P_BCR(void)
{
  ...
  if (R1 == 15)
  {
    ...
    else
    {
      if (R2 != 0)
      {
        waddstr(wcyan, "go to address = 0 or terminate program trace after pressing key"); // переход по адресу = 0 или завершение трассировки программы после нажатия клавиши
	wrefresh(wcyan);
	ret = 1;
      }
    }
...
int P_AH()                                         /*  п р о г р а м м а     */
 ...
 {                                                /*команды A               */
  ...
  ADDR = VR[B] + VR[X] + D;                       /*вычисление абс.адреса и */
  sm = ( int ) ( ADDR -I );                       /*смещения                */
  ARG = OBLZ[BAS_IND + CUR_IND + sm + 2 ] * 0x100 +     /*лашениях ЕС ЭВМ         */
   OBLZ[BAS_IND + CUR_IND + sm + 3 ];              /*                        */
						  /*и                       */
  VR[R1] = VR[R1] + ARG;                          /*сложение с 1-м операндом*/
...
//прокрутка окна
  ...
  waddstr(wcyan, "ready to perform next command with address "); // готовность к выполнению очередной команды с адресом
...
//main programm
  ...
  if ( argc != 2 )
  {
    printf ( "%s\n", "Command line error" ); // Ошибка в командной строке
    return -1;
  }
...
CONT2:
  ...
  endwin(); 
  END:
  printf ("\n%s\n", "processing complete"); // завершение обработки

  return 0;
//Б Л О К  выдачи диагностических сообщений
ERR1:
  printf ("%s%s\n", "error opening file with list of build ", "modules"); // ошибка открытия файла со списком собираемых модулей
  goto END;

ERR2:
  printf ("%s\n", "empty file with list of build modules"); // пустой файл со списком собираемых модулей
  goto END;

ERR3:
  printf ("%s: %s\n" ,
   "error opening file" , SPISOK [I] ); // ошибка открытия файла
  goto END;

ERR4:
  printf ("%s\n" ,
   "list of build modules overflow error" ); // переполнение списка собираемых модулей
  goto END;

ERR5:
  printf ("%s\n" ,
   "buffer of object maps overflow error"); // переполнение буфера хранения об'ектных карт
  goto END;

ERR6:
  printf ("%s\n" ,
   "invalid command code" ); // недопустимый код команды
  goto END;
  
ERR7:
  printf("interruption - addressing error\n"); // прерывание - ошибка адресации
  goto END;
